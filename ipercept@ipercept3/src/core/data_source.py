...
class BaseDataSource(object):
    ...
    def __init__(self,
                 ...
                 batch_size: int = 32,
                 num_threads: int = min(4, multiprocessing.cpu_count()),
        ...
        with tf.variable_scope(''.join(c for c in self.short_name if c.isalnum())):
            ...
            if testing:
                self._preprocess_queue_capacity = (num_threads + 1) * batch_size
            ...
            if testing:
                self._output_tensors = dict([
                    (label, tensor) for label, tensor in
                    zip(labels, self._preprocess_queue.dequeue_many(self.batch_size))
                ])
            ...
            if not testing:
                ...
                self._staging_area = tf.contrib.staging.StagingArea(
                    ...
                    capacity=2,  # This does not have to be high
                )
