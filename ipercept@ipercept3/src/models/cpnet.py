...
class CPNet(CustomModel):
    ...
    def build_model(self, data_sources: Dict[str, BaseDataSource], mode: str):
        ...
        with tf.variable_scope('conv'):
            x = self.get_conv2d("conv1_64", x, filters=64, padding="valid", data_format=data_format, activation=tf.nn.relu)

            x = self.get_max_pooling2d("maxpool_64", x, pool_size=2, strides=2, padding="valid", data_format=data_format)

            x = self.get_conv2d("conv1_128", x, filters=128, padding="valid", data_format=data_format,
                                activation=tf.nn.relu)

            x = self.get_max_pooling2d("maxpool_128", x, pool_size=2, strides=2, padding="valid",
                                       data_format=data_format)

            x = self.get_conv2d("conv1_256", x, filters=256, padding="valid", data_format=data_format,
                                activation=tf.nn.relu)
