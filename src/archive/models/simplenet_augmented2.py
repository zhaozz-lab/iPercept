...
class SimpLeNetAugmented2(CustomModel):
    ...
    def build_model(self, data_sources: Dict[str, BaseDataSource], mode: str):
        ...
        with tf.variable_scope('conv'):
            x = self.get_conv2d_multi("conv1", 1, x, filters=20, kernel_size=3, strides=1, padding='same', data_format=data_format, activation=tf.nn.relu)

            x = self.get_max_pooling2d('maxpool1', x, pool_size=2, strides=2, padding='same', data_format=data_format)

            x = self.get_conv2d_multi("conv2", 1, x, filters=20, kernel_size=3, strides=1, padding='same', data_format=data_format)

            x = self.get_max_pooling2d('maxpool2', x, pool_size=2, strides=2, padding='same', data_format=data_format)

            x = self.get_conv2d_multi("conv3", 1, x, filters=20, kernel_size=3, strides=2, padding='same', data_format=data_format)

            x = self.get_max_pooling2d('maxpool3', x, pool_size=2, strides=2, padding='same', data_format=data_format)

            x = self.get_conv2d_multi("conv4", 1, x, filters=10, kernel_size=2, strides=2, padding='same', data_format=data_format)
        ...
        with tf.variable_scope('fc'):
            ...
            with tf.variable_scope('dropout0'):
                x = tf.layers.dropout(x, rate=0.4, training=mode == tf.estimator.ModeKeys.TRAIN)

            # FC layer
            x = tf.layers.dense(x, units=300, name='fc500-1', activation=tf.nn.relu)
